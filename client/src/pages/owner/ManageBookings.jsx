    ...
    <div className='px-4 md:px-10 pt-10 w-full '>
      ...
      <div className='max-w-3xl w-full rounded-md overflow-hidden border border-borderColor mt-6'>
        ...
        <table className='w-full border-collapse text-left text-sm text-gray-600'>
          ...
          <tbody>
            {bookings.filter(booking => booking?.car).map((booking) => (
              <tr key={booking._id} className='border-t border-borderColor text-gray-500'>
                <td className='p-3 flex items-center gap-3'>
                  <img src={booking.car?.image || assets.carPlaceholderImage} alt="" className='h-12 w-12 aspect-square rounded-md object-cover' />
